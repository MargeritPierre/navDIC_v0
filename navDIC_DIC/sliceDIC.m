...
% Time-Direction of the DIC (if == -1 : reverse DIC)
    dicDir = 1 ;
    refFrame = 1 ;
...
%% PREPARE THE CORRELATION POINTS
        ...
        switch dicDir
            ...
            case -1
                % Get the last full valid time (maximum of valid points)
                    ...
                    refFrame = find(validPts==maxValidPts,1,'last') ;
        end
        
    % Plot the pixel intensities on this line
        refValues = IMG(:,refFrame) ;
        filtVal = conv(refValues,kern/sum(kern),'same') ;
        plot(refValues) ;
        plot(filtVal) ;
    ...
    % Detect interest points (local minimas)
        ...
        AutoNodes = find(abs(d1(2:end-1))<abs(d1(1:end-2)) & abs(d1(2:end-1))<abs(d1(3:end)) & d2(2:end-1)>0)+2 ;
        
    % Create interactive points
        nodePoints = impoint.empty ;
        for n = 1:length(AutoNodes)
            nodePoints(end+1) = impoint(gca,[AutoNodes(n) filtVal(AutoNodes(n))]) ;
        end
        
    % Display informations
        nNodes = numel(AutoNodes) ;
        disp(newline)
        disp(['Number of Nodes Identified: ',num2str(nNodes)])
        disp(['Points Distance: ',num2str(median(diff(AutoNodes)))...
                ,'pix [',num2str(min(diff(AutoNodes)))...
                ,';',num2str(max(diff(AutoNodes)))...
                ,']' ...
                ])
            
%% VALIDATE THE NODES

    nodePoints = nodePoints(isvalid(nodePoints)) ;
    Nodes = [] ;
    for n = 1:numel(nodePoints)
        Nodes(end+1) = round(getPosition(nodePoints(n))*[1;0]) ;
    end
    nNodes = length(Nodes) ;
            

end % RUN THE CODE FROM HERE
%% PERFORM DIC
    ...
    % INITIALIZATION
        ...
        % Dic Frames
            switch dicDir
                case 1
                    processedFrames = refFrame+1:1:size(dicIMG,2) ;
                case -1
                    processedFrames = refFrame-1:-1:1 ;
            end
        
                
    % RUN !
        for fr = processedFrames
            % Import and display image
                lin2 = dicIMG(:,fr) ;
            % Init
                it = 0 ;
                outFlag = false ;
                if fr-dicDir>0 && fr-dicDir<=size(Un,2)
                    Un(:,fr) = Un(:,fr-dicDir) ;
                end
            % Add the previous "speed" as convergence help
                if (dicDir==1 && fr>refFrame+1) || (dicDir==-1 && fr<refFrame-1)
                    Un(:,fr) = Un(:,fr) - dicDir * (Un(:,fr-2*dicDir)-Un(:,fr-dicDir)) * 0;%.1 ;
                end
            % Newton-Raphson
                RMSE_0 = Inf ;
                lastPlotTime = tic ;
                while ~outFlag && ~stopBtn.Value
                    ...
                    % IMAGE FUNCTIONAL
                        ...
                        %diffImg = (lin1mz(:)-lin2wmz(:)).*(WEIGHT*normLin1) ; % Normalized Zero-mean difference
    ...
    % New Reference Configuration
        lastValidFr = sum(isnan(Un(:,1:refFrame)),2)+1 ;
