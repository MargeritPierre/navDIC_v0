...
% Parameters
    ...
    % Crosses
        ...
        AngleShift = repmat([1;-1]*5*pi/180,nNodes,1) ; 60*pi/180 ;
...
% Create crosses
    ...
    % Instantaneous Position shift
        for p = 1:nMembers % the last one will be the center node
            theta = AngleShift(:)+AngleMembers(p) ;
            % Translate 
                hd.Seeds(newSeedNumber).MovingPoints((1:nNodes)+(p-1)*nNodes,:,:) = L*[cos(theta) sin(theta)] + hd.Seeds(newSeedNumber).MovingPoints((1:nNodes)+(p-1)*nNodes,:,:) ;
        end
    % Other point data
